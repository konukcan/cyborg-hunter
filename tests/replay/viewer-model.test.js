...
describe('buildViewerModel — §3 segment time origin', () => {
  ...
  it('rebasing is exact: origin + relative t === the wire t, even before the origin', () => {
    // A segment whose first event precedes its origin (t_load fires after an
    // early event). The rebase must stay a pure subtraction rather than
    // clamping at 0 — the checkpoint executor converts in both directions.
...
describe('buildViewerModel — keyframe spans and defects', () => {
  it('marks a continuation that precedes every keyframe as a defect, and does not throw', () => {
    const model = buildViewerModel(minimalRecording({
      segments: [
        segment({ index: 0, t_start: 0, events: [{ type: 'mouse.move', t: 5 }] }),
